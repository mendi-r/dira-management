...
import { PlusCircle, Edit2, Trash2, Gauge, TrendingUp } from 'lucide-react'
...
// Simple SVG bar chart for consumption history
function ConsumptionChart({ data, sug }) {
  ...
  const vals = data.map(d => {
    const c = Number(d.kriah_nochchi??0) - Number(d.kriah_kodem??0)
    return Math.max(0, c)
  }).filter((_,i) => !data[i].is_kria_ptika)
    ...
    <div className="overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H+30}`} className="w-full max-w-xs">
        {vals.map((v, i) => {
          ...
          return (
            <g key={i}>
              <rect x={x} y={H - h + 5} width={barW} height={h}
                fill={sug==='חשמל'?'#f59e0b':sug==='מים'?'#3b82f6':'#10b981'} rx="2" opacity="0.8"/>
...
export default function Monim() {
  ...
  function calcConsumption(prev, curr) {
  ...
  function openEdit(r){
    ...
    if (r.dirot_id) loadChart(r.dirot_id, r.sug)
    setModal(true)
  }
  ...
  async function save() {
    ...
    setSaving(true)
    const payload = {
      dirot_id: form.dirot_id, sug: form.sug,
      ...
      taarich: form.taarich||null, heara: form.heara,
    }
    const isNew = !form.id
    const { error } = isNew
  ...
  // Totals by type
  const totalChashmal = filtered.filter(r=>r.sug==='חשמל').reduce((s,r)=>s+(calcConsumption(r.kriah_kodem,r.kriah_nochchi)??0),0)
  const totalMayim    = filtered.filter(r=>r.sug==='מים').reduce((s,r)=>s+(calcConsumption(r.kriah_kodem,r.kriah_nochchi)??0),0)

  const columns = [
    { key:'dirot',         label:'דירה',      render:v=>v?`${v.ktovet??''}`:'—' },
    { key:'sug',           label:'סוג',        render:v=><Badge color={SUG_COLORS[v]??'gray'}>{v??'—'}</Badge> },
    { key:'is_kria_ptika', label:'קריאה',     render:v=><span className="text-xs">{v?'פתיחה':'המשך'}</span> },
    { key:'kriah_kodem',   label:'קודמת' },
    { key:'kriah_nochchi', label:'נוכחית' },
    { key:'_consumption',  label:'צריכה',      render:(_,row) => {
      if (row.is_kria_ptika) return <span className="text-slate-400 text-xs">קריאת פתיחה</span>
      const c = calcConsumption(row.kriah_kodem, row.kriah_nochchi)
      return c!==null ? <span className="font-bold text-teal-700">{c}</span> : '—'
    }},
  ...
  return (
    <div className="space-y-4 fade-in">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-48"><SearchInput value={search} onChange={setSearch} placeholder="כתובת, סוג..."/></div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={form.id?'עריכת קריאה':'קריאה חדשה'} size="lg">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="דירה" required>
            <Select value={form.dirot_id??''} onChange={e=>{setForm(f=>({...f,dirot_id:e.target.value}));loadChart(e.target.value,form.sug)}}>
              <option value="">-- בחר דירה --</option>
              {dirot.map(d=><option key={d.id} value={d.id}>{d.ktovet}{d.ir?`, ${d.ir}`:''}</option>)}
            </Select>
          </FormField>
          <FormField label="סוג מונה">
            <Select value={form.sug??'חשמל'} onChange={e=>{setForm(f=>({...f,sug:e.target.value}));loadChart(form.dirot_id,e.target.value)}}>
              <option value="חשמל">חשמל</option>
              <option value="מים">מים</option>
              <option value="גז">גז</option>
          ...
          <div className="sm:col-span-2 flex items-center gap-2">
            <input type="checkbox" id="is_kria_ptika" checked={!!form.is_kria_ptika}
              onChange={e=>setForm(f=>({...f,is_kria_ptika:e.target.checked}))} className="w-4 h-4 text-teal-600 rounded"/>
            <label htmlFor="is_kria_ptika" className="text-sm text-slate-700">קריאת פתיחה (ראשונה לדירה/מונה זה)</label>
          </div>
          {!form.is_kria_ptika && (
